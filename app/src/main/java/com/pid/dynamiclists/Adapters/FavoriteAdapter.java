        ...
        ViewHolder(View view) {
            ...
            view.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    NetworkService.getInstance().getJSONApiUNN().getAbiturient(favorites.get(getAdapterPosition()).getDan().get(5)).enqueue(new Callback<StudentInfoObject>() {
                        @Override
                        public void onResponse(Call<StudentInfoObject> call, Response<StudentInfoObject> response){
                            System.out.println(response.body());
                            ((MainActivity)context).getSupportFragmentManager().beginTransaction().setCustomAnimations(R.anim.fade_in, R.anim.fade_out).replace(R.id.container2, new StudentFragment(response.body(),favorites.get(getAdapterPosition()).getDan().get(5)), "StudentFragment_Tag").addToBackStack("Favorite").commit();
                        }
